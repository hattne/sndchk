...
//        size_t j;
        ...
        /* XXX Only attempt to lookup completely matching
         * fingerprints.  Could be incorporated into the loop
         * condition.
         */
/*
        printf("Releasegroup %zd has %zd members %zd [%zd] observations\n",
               i,
               candidate_releasegroups[i].nmemb,
               candidate_releasegroups[i].nobs,
               fp_number_of_matches(rpn));
        if (candidate_releasegroups[i].nobs < fp_number_of_matches(rpn))
            break;
*/

        for ( ; ; /* j = 0; j < candidate_releasegroups[i].nmemb; j++ */) {
            const char *id = "d101ccf3-3cd4-4ad2-a7a1-aa517c896672"; //candidate_releasegroups[i].releases[j].id;


            /* Check that it is a CD release XXX Should probable
             * return the format(s) of the release (or of any
             * particular medium of the release), and verify here that
             * it's a CD.
             *
             * XXX Note that release
             * be775637-15bd-3769-843c-99d4bd5f9c4b does not have a
             * format assigned.  Fixed that at MusicBrainz 2014-10-21.
             */
//            if (!fp_release_is_cd(rpn, id))
//                continue;


            /* Order the streams w.r.t. to the release.
             */
//            ssize_t *indices;
//            size_t n_indices;
//            if (fp_order_tracks(rpn, id, &indices, &n_indices) != 0) {
//                printf("*** fp_order_tracks() failed\n");
//                continue;
//            }


            /* XXX Could cycle musicbrainz and accuraterip.  Use
             * compartementalization algorithm on previous result to
             * find best split (greedy approach is easiest?).
             * Maximize coverage, minimize overlap (+area and -area),
             * minimize number releases referenced, maximize score.
             *
             * Could check that tracklist is compatible with what
             * we've got before sending off to AccurateRip?
             *
             * If stuff doesn't match properly, it's not an album, so
             * fall back on general approach.
             *
             * XXX Don't look up a release if it's in the same
             * releasegroup as a previous release?
             *
             * XXX Sure we can't do lookup by releasegroups?
             *
             * See https://wiki.musicbrainz.org/Development/XML_Web_Service/Version_2 and https://lucene.apache.org/core/4_3_0/queryparser/org/apache/lucene/queryparser/classic/package-summary.html#package_description for search.
             *
             * See also http://picard.musicbrainz.org/docs/: this tool
             * works on a "cluster" (Picard speek), it really
             * duplicates Picard's scan, but adds the possibility to
             * verify against AccurateRip (so I could have written a
             * plugin).
             */
            try {
            //std::string RecordingID("7b968e1b-7066-4354-85ce-41e9fe7de5e8");
                printf("sending the query\n");

                std::string id2("b3136897-c568-3f62-94af-b98ae7a79abc");

//                MusicBrainz5::CMetadata metadata = Query.Query(
//                    "release", id, "", prutt);

                MusicBrainz5::CQuery::tParamMap prutt2;
                prutt2["release-group"] = "b3136897-c568-3f62-94af-b98ae7a79abc";
                prutt2["inc"] = "media recordings";

                MusicBrainz5::CQuery::tParamMap prutt3;
                prutt3["recording"] = "3de6ea6f-1828-4129-bbf1-62d0cfa76895";
                ...
                for (int j = 0; j < rl->NumItems(); j++) {
                    ...
                    for (int k = 0; k < ml->NumItems(); k++) {
                        ...
                        for (int l = 0; l < tl->NumItems(); l++) {
                            ...
                            MusicBrainz5::CRecording *rec = t->Recording();
                            printf("    Got track %p recording %s\n", t, rec->ID().c_str());
                        }
                    }
                }
                ...
                printf("Got #2 r %p\n", metadata.ReleaseList());
                printf("Got #3 r %p\n", metadata.ReleaseGroupList());
                printf("Got #4 r %p\n", metadata.ReleaseGroup());

                //MusicBrainz5::CReleaseGroup* rg = r->ReleaseGroup();
                MusicBrainz5::CMediumList* ml; //= r->MediumList();
...
//                          << " ITEMS " << ml->NumItems() << std::endl;
                ...
                /* It's match, don't process any further.
                 */
                if (ret == 0)
                    break;


                /* Warn if NumItems() aint't 1!  Appears NumItems
                 * returns int!
                 *
                 * XXX This won't work, because all the recordingids
                 * are lost!  Should then check that this approach can
                 * actually delete all the items from the mask!
                 *
                 * See https://musicbrainz.org/doc/Disc_ID,
                 * http://musicbrainz.org/doc/Disc_ID_Calculation, and
                 * the links therein.
                 */
...
*/
                    ...
                    for (int l = 0; l < tl->NumItems(); l++) {
                        ...
                        if (rsit == rs.end())
                            printf("Failed to locate track %d\n", t->Position());
#endif
                    }

                    std::cout << "  Total length " << total_length
                        //<< " diff " << d->Sectors() - total_length
                              << std::endl;
#endif
                }
            }
...
//    acoustid_free(ac);
    ...
    std::string TrackID("fed0e0e6-c0b8-45be-961c-3a91e295c655");
    std::string RecordingID("7b968e1b-7066-4354-85ce-41e9fe7de5e8");
    std::string fingerprint_str(/* requests->fingerprint */); // XXX

//    MusicBrainz5::CQuery Query("cdlookupexample-1.0");
    try
    {
        MusicBrainz5::CQuery::tParamMap prutt;
//        prutt["inc"] = "release-groups";
        prutt["inc"] = "releases";

        //MusicBrainz5::CMetadata Metadata=Query.Query("discid", DiscID);
        //MusicBrainz5::CMetadata Metadata=Query.Query("recording", RecordingID);
        MusicBrainz5::CMetadata Metadata=Query.Query("recording", RecordingID, "", prutt);

        std::cout << "HATTNE checking 1... " << Metadata << std::endl;
        std::cout << "HATTNE checking 2... " << Metadata.Recording() << std::endl;
        std::cout << "HATTNE checking 3... " << Metadata.Release() << std::endl;
        std::cout << "HATTNE checking 4... " << Metadata.ReleaseGroup() << std::endl;
